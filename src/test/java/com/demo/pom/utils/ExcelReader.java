			...
			try (Workbook workbook = WorkbookFactory.create(inputStream)) {
				...
				for (int cellIndex = 0; cellIndex < headerRow.getLastCellNum(); cellIndex++) {
					String key = formatter
							.formatCellValue(headerRow.getCell(cellIndex, Row.MissingCellPolicy.CREATE_NULL_AS_BLANK))
							.trim();
					String value = formatter
							.formatCellValue(dataRow.getCell(cellIndex, Row.MissingCellPolicy.CREATE_NULL_AS_BLANK))
							.trim();
